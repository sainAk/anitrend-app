...
import com.mxt.anitrend.extension.visible
...
class MediaOverviewFragment : FragmentBase<Media, MediaPresenter, Media>() {
    ...
    private val youtubePlayerFragment by lazy {
        YoutubePlayerFragment.newInstance(model?.trailer)
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        if (arguments != null) {
            mediaId = arguments?.getLong(KeyUtil.arg_id) ?: -1
    ...
    override fun updateUI() {
        if (activity != null && model?.trailer != null && model?.trailer?.site == "youtube") {
            if (YouTubeIntents.canResolvePlayVideoIntent(activity)) {
                childFragmentManager.beginTransaction()
                        .replace(R.id.youtube_view, youtubePlayerFragment as Fragment)
                        .commit()
            } else {
                childFragmentManager.beginTransaction()
        ...
        if (model?.tags != null && model?.tagsNoSpoilers != null) {
            if (model?.tagsNoSpoilers?.size == model?.tags?.size)
                binding?.showSpoilerTags?.gone()
            else
                binding?.showSpoilerTags?.visible()
        }

        if (genreAdapter == null) {
            genreAdapter = GenreAdapter(context)
            genreAdapter?.onItemsInserted(presenter.buildGenres(model))
            genreAdapter?.clickListener = object : ItemClickListener<Genre> {
                /**
                ...
                override fun onItemClick(target: View, data: IntPair<Genre?>) {
                    when (target.id) {
                        R.id.container -> {
                            data.second?.also { model ->
                                activity.startNewActivity<MediaBrowseActivity>(Bundle().apply {
                                    putString(KeyUtil.arg_activity_tag, model.genre)
                                    putParcelable(
                                        KeyUtil.arg_graph_params,
                                        GraphUtil.getDefaultQuery(true)
                                            .putVariable(KeyUtil.arg_type, mediaType)
                                            .putVariable(KeyUtil.arg_genres, model.genre)
                                    )
                                    putParcelable(
        ...
        if (tagAdapter == null) {
            ...
            tagAdapter?.clickListener = object : ItemClickListener<MediaTag> {
                /**
                 * When the target view from [View.OnClickListener]
                 * is clicked from a view holder this method will be called
                 *
                        ...
                        R.id.container -> {
                            data.second?.also { model ->
                                DialogUtil.createTagMessage(
                                    activity, model.name, model.description, model.isMediaSpoiler,
                                    R.string.More, R.string.Close, object: DialogCallback {
                                        override fun invoke(p1: MaterialDialog) {
                                            activity.startNewActivity<MediaBrowseActivity>(Bundle().apply {
                                                putString(KeyUtil.arg_activity_tag, model.name)
                                                putParcelable(
                                                    KeyUtil.arg_graph_params,
                                                    GraphUtil.getDefaultQuery(true)
                                                        .putVariable(KeyUtil.arg_type, mediaType)
                                                        .putVariable(KeyUtil.arg_tags, model.name)
                                                )
                                                putParcelable(
                                                    KeyUtil.arg_media_util,
                                                    MediaBrowseUtil().setCompactType(true)
                                                        .setBasicFilter(true).setFilterEnabled(true)
                                                )
                                            })
                                        }
                                    }
    ...
    override fun makeRequest() {
        ...
        viewModel.requestData(KeyUtil.MEDIA_OVERVIEW_REQ, context!!)
    }
